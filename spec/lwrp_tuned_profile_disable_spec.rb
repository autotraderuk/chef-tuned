...
describe 'test_tuned::disable' do
  ...
  platforms.each do |platform, versions|
    versions.each do |version|
      context "on #{platform.capitalize} #{version}" do
        ...
        it 'creates resource :disable tuned_profile[default]' do
          expect(chef_run).to disable_tuned_profile('default')
        end

        it 'disables tuned profile via execute' do
          expect(chef_run).to run_execute('tuned-adm off')
        end
      end
    end
  end
